...
import { WifiOff, RefreshCw, Loader2, Wifi, Info, AlertCircle } from 'lucide-react';
...
const ChatConnectionStatus = () => {
  ...
  const StatusWrapper = ({ icon: Icon, color, label, children }) => (
    <div className="flex flex-col w-full">
      <div className={`flex items-center ${color} text-sm rounded px-2 py-1 relative`}>
      ...
      {showDetails && (
        <div className="text-xs p-2 mt-1 bg-gray-50 rounded border border-gray-200 text-gray-600">
          <p><span className="font-medium">State:</span> {connectionState}</p>
          <p><span className="font-medium">Last change:</span> {formatRelativeTime(lastStateChange)}</p>
          {lastMessageTime && (
            <p><span className="font-medium">Last message:</span> {formatRelativeTime(lastMessageTime)}</p>
          )}
          {reconnectAttempt > 0 && (
            <p><span className="font-medium">Reconnect attempts:</span> {reconnectAttempt}</p>
          )}
          {hasError && (
            <p className="text-red-500"><span className="font-medium">Last error:</span> {lastError || 'Connection error'}</p>
          )}
        </div>
      )}
    </div>
  ...
  if (connectionState === 'disconnected' && reconnectAttempt > 0) {
    return (
      <StatusWrapper 
        ...
        label="Reconnecting..."
      >
        <span className="ml-1 text-xs bg-yellow-200 text-yellow-800 px-1 rounded-sm">{reconnectAttempt}</span>
      </StatusWrapper>
    );
  }
  ...
  if (isPolling) {
    return (
      ...
      >
        <span className="ml-1 text-xs bg-blue-200 text-blue-800 px-1 rounded-sm">Polling</span>
      </StatusWrapper>
    );
  }
