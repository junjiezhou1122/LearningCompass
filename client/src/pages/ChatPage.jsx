...
const ChatMessage = ({ message, isCurrentUser }) => {
  ...
  return (
    <div className={`relative flex items-start mb-1.5 py-1 px-2 group hover:bg-orange-50/70 rounded ${isCurrentUser ? 'justify-end' : 'justify-start'}`}>
      {!isCurrentUser && (
      ...
      <div className={`max-w-[75%]`}>
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <span className="text-xs text-orange-400 ml-2 cursor-default">{formattedTime}</span>
                </TooltipTrigger>
        ...
        <div className={`px-4 py-2 rounded-lg shadow-sm ${isCurrentUser 
        ...
        {isCurrentUser && (
          <div className="text-xs text-orange-400 mt-1 text-right mr-1">
            ...
            {message.isRead && <span className="ml-1 text-orange-500">• Read</span>}
          </div>
        )}
      </div>
      ...
      <div className="absolute right-2 top-2 opacity-0 group-hover:opacity-100 transition-opacity">
        <div className="flex space-x-1 bg-white rounded-md border border-orange-200 shadow-sm p-1">
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" className="h-7 w-7 text-orange-500 hover:bg-orange-50 rounded-full">
...
const Channel = ({ name, isActive, unreadCount, onClick }) => {
  return (
    <div 
      className={`flex items-center py-1.5 px-2 rounded-md mb-1 cursor-pointer shadow-sm ${isActive ? 'bg-orange-600 text-white' : 'text-orange-800 hover:bg-orange-100 hover:text-orange-900'}`}
      onClick={onClick}
    >
...
const DirectMessage = ({ user, isActive, isOnline, unreadCount, onClick }) => {
  return (
    <div 
      className={`flex items-center py-1.5 px-2 rounded-md mb-1 cursor-pointer ${isActive ? 'bg-orange-600 text-white' : 'text-orange-800 hover:bg-orange-100 hover:text-orange-900'}`}
      onClick={onClick}
    >
      <div className="relative mr-2">
        <Avatar className="h-7 w-7 border border-orange-100 shadow-sm">
          <AvatarFallback className={`${isActive ? 'bg-orange-800 text-white' : 'bg-orange-200 text-orange-800'} text-xs font-medium`}>
...
const ChatPage = () => {
  ...
  }, [hasMore, isLoadingMore, activeChat, token, page, MESSAGES_PER_PAGE, toast]);

  // Primary function to load messages for a chat conversation
  // Uses pagination to load only the most recent messages first
  const loadMessages = useCallback(async (partnerId, isLoadingOlder = false) => {
    if (!token) return;
  ...
  return (
    <div className="h-full flex flex-col bg-orange-50">
      ...
      <div className="h-10 border-b border-orange-200 flex items-center justify-between px-4 z-10 bg-orange-100 text-orange-800 shadow-sm">
        ...
        <div className="flex items-center space-x-1">
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" className="text-orange-700 hover:bg-orange-200 h-7 w-7 rounded-md">
      ...
      <div className="flex-1 flex overflow-hidden chat-container">
        {/* Sidebar */}
        <div 
          className={`w-64 bg-orange-50 border-r border-orange-200 flex flex-col ${isMobileSidebarOpen ? 'block' : 'hidden'} lg:block overflow-hidden`}
            ...
            <div className="p-3">
              <div className="mb-6">
                <div className="flex items-center justify-between mb-2 px-1">
                  <h3 className="text-xs font-semibold text-orange-700 uppercase tracking-wider">Direct Messages</h3>
                  <Button variant="ghost" size="icon" className="h-5 w-5 text-orange-500 hover:text-orange-700">
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-2 px-1">
                  <h3 className="text-xs font-semibold text-orange-700 uppercase tracking-wider">Channels</h3>
                  <Button variant="ghost" size="icon" className="h-5 w-5 text-orange-500 hover:text-orange-700">
                    <PlusCircle className="h-4 w-4" />
                  </Button>
                </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden bg-white">
          {activeChat ? (
            <>
              ...
              {/* Messages area */}
              <ScrollArea ref={scrollAreaRef} className="flex-1 overflow-y-auto messages-container bg-white">
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center empty-state-container bg-white">
              <div className="max-w-md">
                ...
                <h2 className="text-xl font-semibold mb-2 text-orange-800">Your Messages</h2>
                <p className="text-orange-600 mb-6 text-sm">
                  Send private messages to friends and colleagues. Select a conversation or start a new one.
                </p>
              ...
              {!connected && (
                <div className="mt-8 p-4 bg-orange-100 text-orange-700 rounded-md flex items-center">
                  ...
                  <p className="text-sm">Connection to chat server lost. Trying to reconnect...</p>
                </div>
              )}
            </div>
          )}
