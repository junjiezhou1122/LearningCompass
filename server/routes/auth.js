...
router.post("/phone", async (req, res) => {
  try {
    ...
    const { token } = req.body;
    console.log(
      "Received Phone auth request with token:",
      token ? "Token exists" : "No token"
    );
    ...
    console.log("Verified Phone user with UID:", uid, "Phone:", phoneNumber);

    if (!phoneNumber) {
      console.error("No phone number found in decoded token");
      return res
        .status(400)
        .json({ message: "No phone number found in authentication" });
    }

    // Check if the user already exists in the database by provider ID or phone number
    let dbUser;
    try {
      dbUser = await db.query.users.findFirst({
      ...
      // If not found by providerId, try by phone number
      if (!dbUser) {
        dbUser = await db.query.users.findFirst({
    ...
    // If user doesn't exist, create a new one
    if (!dbUser) {
      console.log("Creating new user for Phone sign-in:", phoneNumber);

      // Generate a random password for Phone users
      const randomPassword = Math.random().toString(36).slice(-12);
      const hashedPassword = await bcrypt.hash(randomPassword, 10);

      // Create username from phone number
      const username = `user_${phoneNumber.slice(-4)}_${uid.substring(0, 6)}`;
      ...
      const newUser = {
        ...
        password: hashedPassword,
        email: `${username}@placeholder.com`, // Adding placeholder email for schema compatibility
        createdAt: new Date().toISOString(),
        authProvider: "phone",
        providerId: uid,
      };
      ...
      try {
        ...
        dbUser = insertResult[0];
        console.log(
          "Created new user from Phone sign-in:",
          dbUser.username,
          "with ID:",
          dbUser.id
        );
      ...
      console.log(
        "Existing user signed in with Phone:",
        dbUser.username,
        "with ID:",
        dbUser.id
      );
      ...
      if (!dbUser.providerId) {
        try {
          ...
          console.log("Updated existing user with Phone provider ID");
        } catch (updateError) {
          console.error("Error updating user with provider ID:", updateError);
          // Continue with login even if update fails
        }
    ...
    console.log("Sending successful Phone auth response");
    return res.status(200).json(responseData);
  } catch (error) {
