import { 
  ...
  chatGroupMessages, ChatGroupMessage, InsertChatGroupMessage,
  chatGroupInvitations, ChatGroupInvitation, InsertChatGroupInvitation,
  chatGroupApplications, ChatGroupApplication, InsertChatGroupApplication,
...
export interface IStorage {
  ...
  getChatGroupMember(id: number): Promise<ChatGroupMember | undefined>;
  getChatGroupMemberByUserAndGroup(userId: number, groupId: number): Promise<ChatGroupMember | undefined>;
  ...
  updateChatGroupMember(id: number, data: Partial<InsertChatGroupMember>): Promise<ChatGroupMember | undefined>;
  ...
  getChatGroupMessage(id: number): Promise<ChatGroupMessage | undefined>;
  getChatGroupMessages(groupId: number, options?: {
    limit?: number;
    offset?: number;
    sinceDatetime?: Date;
  }): Promise<(ChatGroupMessage & { sender: User })[]>;
  createChatGroupMessage(message: InsertChatGroupMessage): Promise<ChatGroupMessage>;
  getGroupUnreadMessageCount(userId: number): Promise<number>;
  updateGroupLastRead(groupId: number, userId: number): Promise<boolean>;
  
  // Group Chat Invitations operations
  getChatGroupInvitation(id: number): Promise<ChatGroupInvitation | undefined>;
  getChatGroupInvitations(groupId: number): Promise<(ChatGroupInvitation & { invitee: User; inviter: User })[]>;
  getPendingChatGroupInvitationsByUser(userId: number): Promise<(ChatGroupInvitation & { group: ChatGroup; inviter: User })[]>;
  createChatGroupInvitation(invitation: InsertChatGroupInvitation): Promise<ChatGroupInvitation>;
  respondToChatGroupInvitation(id: number, status: string): Promise<ChatGroupInvitation | undefined>;
  deleteChatGroupInvitation(id: number): Promise<boolean>;
  
  // Group Chat Applications operations
  getChatGroupApplication(id: number): Promise<ChatGroupApplication | undefined>;
  getChatGroupApplications(groupId: number): Promise<(ChatGroupApplication & { applicant: User })[]>;
  getPendingChatGroupApplicationsByUser(userId: number): Promise<(ChatGroupApplication & { group: ChatGroup })[]>;
  getPendingChatGroupApplicationsForGroups(groupIds: number[]): Promise<(ChatGroupApplication & { group: ChatGroup; applicant: User })[]>;
  createChatGroupApplication(application: InsertChatGroupApplication): Promise<ChatGroupApplication>;
  respondToChatGroupApplication(id: number, status: string, responderId: number): Promise<ChatGroupApplication | undefined>;
  deleteChatGroupApplication(id: number): Promise<boolean>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async getChatGroups(options?: {
    ...
    if (options?.creatorId) {
      whereConditions.push(eq(chatGroups.creatorId, options.creatorId));
    }
    
    if (options?.isPrivate !== undefined) {
      whereConditions.push(eq(chatGroups.isPrivate, options.isPrivate));
    }
    
    if (options?.search) {
      whereConditions.push(
        or(
          like(chatGroups.name, `%${options.search}%`),
          like(chatGroups.description, `%${options.search}%`)
        )
      );
    }
    
    // If userId is provided, we want to get groups where the user is a member
    if (options?.userId) {
      // We need to handle this differently because we need to join with chat_group_members
      const userGroups = await db.select({
        groupId: chatGroupMembers.groupId
      })
      .from(chatGroupMembers)
      .where(eq(chatGroupMembers.userId, options.userId));
      
      const userGroupIds = userGroups.map(g => g.groupId);
      
      if (userGroupIds.length > 0) {
        whereConditions.push(inArray(chatGroups.id, userGroupIds));
      } else {
        // If user is not a member of any group, return empty array
        return [];
      }
    }
    ...
    query = query.orderBy(desc(chatGroups.createdAt));
    
    return await query;
  }
  
  async createChatGroup(group: InsertChatGroup): Promise<ChatGroup> {
    const [result] = await db.insert(chatGroups)
      .values(group)
      .returning();
  ...
  // Group Chat Members operations
  async getChatGroupMember(id: number): Promise<ChatGroupMember | undefined> {
  ...
  async getChatGroupMemberByUserAndGroup(userId: number, groupId: number): Promise<ChatGroupMember | undefined> {
    const [member] = await db.select()
      .from(chatGroupMembers)
      .where(
        and(
  ...
  async createChatGroupMember(member: InsertChatGroupMember): Promise<ChatGroupMember> {
    ...
    const existingMember = await this.getChatGroupMemberByUserAndGroup(member.userId, member.groupId);
  ...
  async updateChatGroupMember(id: number, data: Partial<InsertChatGroupMember>): Promise<ChatGroupMember | undefined> {
  ...
  async isGroupMember(groupId: number, userId: number): Promise<boolean> {
    const member = await this.getChatGroupMemberByUserAndGroup(userId, groupId);
    return !!member;
  }
  ...
  async getUserGroups(userId: number): Promise<ChatGroup[]> {
    ...
    return memberGroups.map(g => g.group);
  }
  
  // Group Chat Messages operations
  async getChatGroupMessage(id: number): Promise<ChatGroupMessage | undefined> {
    const [message] = await db.select()
      .from(chatGroupMessages)
      .where(eq(chatGroupMessages.id, id));
    return message || undefined;
  }
  
  async getChatGroupMessages(groupId: number, options?: {
    limit?: number;
    offset?: number;
    sinceDatetime?: Date;
  }): Promise<(ChatGroupMessage & { sender: User })[]> {
    ...
    if (options?.sinceDatetime) {
      query = query.where(gte(chatGroupMessages.createdAt, options.sinceDatetime));
    }
    
    // Order by created date (newest first)  
    query = query.orderBy(desc(chatGroupMessages.createdAt));
  ...
  async createChatGroupMessage(message: InsertChatGroupMessage): Promise<ChatGroupMessage> {
    // Check if the sender is a member of the group
    const isMember = await this.isGroupMember(message.groupId, message.senderId);
    
    if (!isMember) {
      throw new Error('Only group members can send messages to the group');
    }
  ...
  async getGroupUnreadMessageCount(userId: number): Promise<number> {
    ...
    // For each group, count messages that were sent after the user's last_read_timestamp
    for (const membership of memberGroups) {
      const lastReadTime = membership.lastReadTimestamp;
      
      let query = db.select({
        count: sql`count(*)`
      })
      .from(chatGroupMessages)
      .where(eq(chatGroupMessages.groupId, membership.groupId));
      
      if (lastReadTime) {
        query = query.where(gt(chatGroupMessages.createdAt, lastReadTime));
      }
  ...
  async updateGroupLastRead(groupId: number, userId: number): Promise<boolean> {
    const result = await db.update(chatGroupMembers)
  ...
  // Group Chat Invitations operations
  async getChatGroupInvitation(id: number): Promise<ChatGroupInvitation | undefined> {
    const [invitation] = await db.select()
      .from(chatGroupInvitations)
      .where(eq(chatGroupInvitations.id, id));
    return invitation || undefined;
  }
  
  async getChatGroupInvitations(groupId: number): Promise<(ChatGroupInvitation & { invitee: User; inviter: User })[]> {
    const invitations = await db.select({
      invitation: chatGroupInvitations,
      invitee: users.as('invitee'),
      inviter: users.as('inviter')
    })
    .from(chatGroupInvitations)
    .innerJoin(users.as('invitee'), eq(chatGroupInvitations.inviteeId, users.as('invitee').id))
    .innerJoin(users.as('inviter'), eq(chatGroupInvitations.inviterId, users.as('inviter').id))
    .where(eq(chatGroupInvitations.groupId, groupId));
  ...
  async getPendingChatGroupInvitationsByUser(userId: number): Promise<(ChatGroupInvitation & { group: ChatGroup; inviter: User })[]> {
    const invitations = await db.select({
      invitation: chatGroupInvitations,
      group: chatGroups,
      inviter: users
    ...
    .from(chatGroupInvitations)
    .innerJoin(chatGroups, eq(chatGroupInvitations.groupId, chatGroups.id))
    .innerJoin(users, eq(chatGroupInvitations.inviterId, users.id))
    .where(
      and(
        eq(chatGroupInvitations.inviteeId, userId),
        eq(chatGroupInvitations.status, 'pending')
      )
    );
  ...
  async createChatGroupInvitation(invitation: InsertChatGroupInvitation): Promise<ChatGroupInvitation> {
    // Check if the inviter is an admin of the group
    const isAdmin = await this.isGroupAdmin(invitation.groupId, invitation.inviterId);
    ...
    const isMember = await this.isGroupMember(invitation.groupId, invitation.inviteeId);
    ...
    const [existingInvitation] = await db.select()
      ...
      .where(
        and(
          eq(chatGroupInvitations.groupId, invitation.groupId),
          eq(chatGroupInvitations.inviteeId, invitation.inviteeId),
          eq(chatGroupInvitations.status, 'pending')
        )
      );
    ...
    const [result] = await db.insert(chatGroupInvitations)
      .values(invitation)
      .returning();
  ...
  async respondToChatGroupInvitation(id: number, status: string): Promise<ChatGroupInvitation | undefined> {
    // Get the invitation
    const invitation = await this.getChatGroupInvitation(id);
    ...
    const [updatedInvitation] = await db.update(chatGroupInvitations)
      .set({
        status,
    ...
    // If accepted, add the invitee as a member of the group
    if (status === 'accepted') {
  ...
  async deleteChatGroupInvitation(id: number): Promise<boolean> {
    const result = await db.delete(chatGroupInvitations)
      .where(eq(chatGroupInvitations.id, id));
  ...
  // Group Chat Applications operations
  async getChatGroupApplication(id: number): Promise<ChatGroupApplication | undefined> {
    const [application] = await db.select()
      .from(chatGroupApplications)
      .where(eq(chatGroupApplications.id, id));
    return application || undefined;
  }
  
  async getChatGroupApplications(groupId: number): Promise<(ChatGroupApplication & { applicant: User })[]> {
    const applications = await db.select({
      application: chatGroupApplications,
      applicant: users
    })
    .from(chatGroupApplications)
    .innerJoin(users, eq(chatGroupApplications.applicantId, users.id))
    .where(eq(chatGroupApplications.groupId, groupId));
  ...
  async getPendingChatGroupApplicationsByUser(userId: number): Promise<(ChatGroupApplication & { group: ChatGroup })[]> {
    const applications = await db.select({
      application: chatGroupApplications,
      group: chatGroups
    })
    .from(chatGroupApplications)
    .innerJoin(chatGroups, eq(chatGroupApplications.groupId, chatGroups.id))
    .where(
      and(
        eq(chatGroupApplications.applicantId, userId),
        eq(chatGroupApplications.status, 'pending')
      )
    );
  ...
  async getPendingChatGroupApplicationsForGroups(groupIds: number[]): Promise<(ChatGroupApplication & { group: ChatGroup; applicant: User })[]> {
    ...
    .where(
      and(
        inArray(chatGroupApplications.groupId, groupIds),
        eq(chatGroupApplications.status, 'pending')
      )
    );
  ...
  async createChatGroupApplication(application: InsertChatGroupApplication): Promise<ChatGroupApplication> {
    // Check if the user is already a member of the group
    const isMember = await this.isGroupMember(application.groupId, application.applicantId);
    
    if (isMember) {
      throw new Error('User is already a member of this group');
    }
    
    // Check if there's already a pending application for this user and group
    const [existingApplication] = await db.select()
      .from(chatGroupApplications)
      .where(
        and(
          eq(chatGroupApplications.groupId, application.groupId),
          eq(chatGroupApplications.applicantId, application.applicantId),
          eq(chatGroupApplications.status, 'pending')
        )
      );
    
    if (existingApplication) {
      throw new Error('An application is already pending for this user');
    }
    
    const [result] = await db.insert(chatGroupApplications)
      .values(application)
      .returning();
  ...
  async respondToChatGroupApplication(id: number, status: string, responderId: number): Promise<ChatGroupApplication | undefined> {
    // Get the application
    const application = await this.getChatGroupApplication(id);
    
    if (!application) {
      return undefined;
    }
    
    // Check if the responder is an admin of the group
    const isAdmin = await this.isGroupAdmin(application.groupId, responderId);
    
    if (!isAdmin) {
      throw new Error('Only group admins can respond to applications');
    }
    
    // Update the application status
    const [updatedApplication] = await db.update(chatGroupApplications)
      .set({
        status,
        respondedAt: new Date(),
        responderId
    ...
    // If approved, add the applicant as a member of the group
    if (status === 'approved') {
      await this.createChatGroupMember({
  ...
  async deleteChatGroupApplication(id: number): Promise<boolean> {
    const result = await db.delete(chatGroupApplications)
      .where(eq(chatGroupApplications.id, id));
